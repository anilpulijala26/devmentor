...
fullCode:
  title: "Secure Server Actions & Dynamic Revalidation API"
  description: "A Next.js Server Action code block illustrating authentication checks, payload validation with Zod, and data cache invalidations."
  language: "typescript"
  code: |
    "use server";
    ...
    const schema = z.object({
      itemId: z.string().uuid("Invalid UUID identifier"),
    ...
    export async function updateInventoryAction(prevState: any, formData: FormData) {
      try {
        ...
        const { itemId, quantity } = validated.data;
        console.log(`Writing to database: User ${user.id} updated Item ${itemId} to ${quantity}`);

        // 3. Invalidate server Data Cache and rebuild static layouts
