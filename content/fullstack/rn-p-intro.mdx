...
1. **Submit Credentials**: The user enters credentials (email/password) on the login form.
2. **Send Request**: The frontend sends credentials to `/api/auth/login`.
3. **Verify & Generate**: The backend verifies credentials against the database and signs a JWT (JSON Web Token).
4. **Token Storage**:
...
6. **Backend Verification**: The backend verifies the token before processing protected routes.
