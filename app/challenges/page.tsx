...
import { ArrowRight, Code2, Award, CheckCircle2, ChevronRight } from "lucide-react";

export const metadata = {
  title: "Coding Challenges - CodeNivra",
  description: "Solve daily coding challenges to build your problem solving skills and coding habits.",
};
...
export default async function ChallengesCatalogPage() {
  ...
  // Fetch all challenges joined with user's attempts
  ...
  // Calculate today's challenge based on date modulo
  const todayIndexRes = await dbQuery(
    `SELECT ((CURRENT_DATE - '2026-07-01'::date) % 10) as offset_val`
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-slate-500 max-w-2xl">
            Build solid problem-solving instincts. Solve one JS problem each day to protect your streak.
          </p>
        </div>
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              <div className="max-w-3xl">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${
                    todayChallenge.difficulty === "Easy" ? "bg-emerald-500/20 text-emerald-300" : "bg-amber-500/20 text-amber-300"
                  }`}>
                    {todayChallenge.difficulty}
                  </span>
                  {todayChallenge.is_solved && (
                    <span className="flex items-center gap-1 bg-emerald-500 text-white px-2.5 py-0.5 rounded-full text-xs font-semibold">
              ...
              <div className="shrink-0">
                <Link
                  ...
                  className="inline-flex items-center gap-2 bg-white text-slate-900 hover:bg-slate-50 transition-colors font-bold px-6 py-3.5 rounded-2xl shadow-lg hover:shadow-xl text-sm"
                >
                  {todayChallenge.is_solved ? "Review Submission" : "Solve Challenge"} <ArrowRight className="w-4 h-4" />
                </Link>
              </div>
            </div>
          </div>
        ...
        {/* All Challenges Catalog Grid */}
        <h2 className="text-lg font-bold text-slate-800 mb-5 flex items-center gap-2">
          <Code2 className="w-5 h-5 text-indigo-500" /> Complete Challenges Archive
        </h2>
              ...
              <div
                ...
                className={`bg-white rounded-2xl border p-5 transition-all flex flex-col justify-between ${
                  isToday
                    ...
                    : "border-slate-200/80 hover:shadow-[0_8px_30px_rgba(15,23,42,0.04)] hover:border-slate-300"
                }`}
              >
                <div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-400 font-bold font-mono">
                      CHALLENGE #{c.order_index}
                    </span>
                    <div className="flex items-center gap-1.5">
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${
                        c.difficulty === "Easy" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"
                      }`}>
                        {c.difficulty}
                      </span>
                      {isToday && (
                        <span className="bg-indigo-50 text-indigo-600 border border-indigo-100 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider">
                          Active
                        </span>
                      )}
                    </div>
                  ...
                  <h3 className="text-base font-bold text-slate-900 mt-3.5 tracking-tight">
                    {c.title}
                  </h3>
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
                  ...
                  {isToday ? (
                    ...
                    >
                      Open Challenge <ChevronRight className="w-3.5 h-3.5" />
                    </Link>
                  ) : (
                    <span className="text-xs text-slate-300 font-semibold cursor-not-allowed">
                      Unlock on cycle
                    </span>
                  )}
                </div>
