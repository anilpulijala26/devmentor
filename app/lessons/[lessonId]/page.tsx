...
export async function generateMetadata({ params }: Props) {
  try {
    ...
    if (res.rows.length > 0) {
      return { title: `${res.rows[0].title} - CodeNivra` };
    }
  } catch {}
  return { title: "Lesson Guide - CodeNivra" };
}
...
export default async function LessonDetailPage({ params }: Props) {
  ...
  const isCompleted = progressResult.rows.length > 0 ? progressResult.rows[0].is_completed : false;
